...
function GameServer() {
    ...
    this.config = { // Border - Right: X increases, Down: Y increases (as of 2015-05-20)
        ...
        playerRecombineSplitTime: 30, // Seconds before normally split cells are allowed to recombine
        playerRecombineRefreshAll: 0, // Every split refreshes split recombine time on all own cells
        playerRecombineVirusTime: 30, // Seconds before virused split cells are allowed to recombine
        playerIgnorCollisionSplitTime: 0, // Seconds before normally split cells start colliding with own cells
        playerIgnorCollisionRefreshAll: 0, // Every split refreshes split collision ignore on all own cells
        playerIgnorCollisionVirusTime: 0.8, // Seconds before virused split cells start colliding with own cells
        playerSplitBoost: 1, // Extra multiplier for normally split cells
        playerSplitTicks: 32, // Amount of physics ticks for normally split cells
        playerSplitDecay: 0.85, // Speed decay per physics tick for normally split cells
        playerSplitStartDistance: 1, // Multiplier for initial split spawn distance from parent
    ...
    if (!this.gameMode) {
        throw new Error("Unknown serverGamemode: " + this.config.serverGamemode);
    }
...
GameServer.prototype.start = function() {
    ...
    function connectionEstablished(ws) {
        ...
        function close(error) {
            ...
            console.log("[Game] Disconnect: " + this.socket.remoteAddress + ":" + this.socket.remotePort + reason);
        ...
        console.log("[Game] Connect: " + ws.remoteAddress + ":" + ws.remotePort);
    }
    // start server status
...
GameServer.prototype.mainLoop = function() {
    ...
    if (this.tick >= 50) {
	    ...
	    setTimeout(this.cellTick.bind(this), 0);
            for (var ri = 0; ri < this.nodesPlayer.length; ri++) {
                var rc = this.nodesPlayer[ri];
                if (rc && rc.recombineTicks > 0) {
                    rc.recombineTicks--;
                }
                if (rc && rc.restoreCollisionTicks > 0) {
                    rc.restoreCollisionTicks--;
                    if (rc.restoreCollisionTicks <= 0) {
                        rc.ignoreCollision = false;
                    }
                }
            }
...
GameServer.prototype.splitCells = function(client) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var size = cell.getSize() * (Number(this.config.playerSplitStartDistance) || 1);
        var startPos = {
            x: cell.position.x + ( size * Math.sin(angle) ),
            y: cell.position.y + ( size * Math.cos(angle) )
        };
        if (startPos.x < this.config.borderLeft) {
            startPos.x = this.config.borderLeft;
        }
        if (startPos.x > this.config.borderRight) {
            startPos.x = this.config.borderRight;
        }
        if (startPos.y < this.config.borderTop) {
            startPos.y = this.config.borderTop;
        }
        if (startPos.y > this.config.borderBottom) {
            startPos.y = this.config.borderBottom;
        }
        // Calculate mass and speed of splitting cell
        var splitBoost = Number(this.config.playerSplitBoost) || 1;
        var splitSpeed = cell.getSpeed() * 6 * splitBoost;
        var newMass = cell.mass / 2;
        cell.mass = newMass;
        // Create cell
        var split = new Entity.PlayerCell(this.getNextNodeId(), client, startPos, newMass);
        split.setAngle(angle);
        split.setMoveEngineData(splitSpeed, Number(this.config.playerSplitTicks) || 32, Number(this.config.playerSplitDecay) || 0.85);
        split.calcMergeTime(this.config.playerRecombineSplitTime);
        if (Number(this.config.playerRecombineRefreshAll)) {
            for (var j = 0; j < client.cells.length; j++) {
                if (client.cells[j]) {
                    client.cells[j].calcMergeTime(this.config.playerRecombineSplitTime);
                }
            }
        }
        split.restoreCollisionTicks = Math.max(0, ((Number(this.config.playerIgnorCollisionSplitTime) || 0) * 20 + 0.5) >> 0);
        split.ignoreCollision = split.restoreCollisionTicks > 0;
        if (Number(this.config.playerIgnorCollisionRefreshAll) && split.restoreCollisionTicks > 0) {
            for (var k = 0; k < client.cells.length; k++) {
                if (client.cells[k]) {
                    client.cells[k].restoreCollisionTicks = split.restoreCollisionTicks;
                    client.cells[k].ignoreCollision = true;
                }
            }
        }
...
GameServer.prototype.newCellVirused = function(client, parent, angle, mass, speed) {
    ...
    newCell.calcMergeTime(this.config.playerRecombineVirusTime);
    newCell.ignoreCollision = true;  // Turn off collision
    newCell.restoreCollisionTicks = Math.max(0, ((Number(this.config.playerIgnorCollisionVirusTime) || 0) * 20 + 0.5) >> 0);
...
WebSocket.prototype.sendPacket = function(packet) {
    ...
    if (this._socket && this._socket.bufferSize > 1024 * 1024) {
        return;
    }
    ...
    try {
        ...
        var packetName = packet && packet.constructor ? packet.constructor.name : "UnknownPacket";
        console.log("[Error] sendPacket " + packetName + ": " + e);
    }
};
