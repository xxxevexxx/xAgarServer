...
PlayerCell.prototype.calcMergeTime = function(base) {
    var ticks = Number(base) * 20;
    this.recombineTicks = Math.max(0, (ticks + 0.5) >> 0);
};
...
PlayerCell.prototype.checkBorder = function(config) {
    if (this.position.x < config.borderLeft) {
        this.position.x = config.borderLeft;
    }
    if (this.position.x > config.borderRight) {
        this.position.x = config.borderRight;
    }
    if (this.position.y < config.borderTop) {
        this.position.y = config.borderTop;
    }
    if (this.position.y > config.borderBottom) {
        this.position.y = config.borderBottom;
    }
};
